...
class SNGP(tf.keras.Model):
    """
    Code is adopted from https://github.com/google/uncertainty-baselines
    
    Citation:
      [1] J. Liu, Z. Lin, S. Padhy, D. Tran, T. Bedrax Weiss, and B. Lakshminarayanan. Simple and
          ...
          Information Processing Systems, volume 33, pages 7498–7512. Curran Associates, Inc., 2020.
    """
    def __init__(self, C, x_dim=20, gp_cov_momentum=-1, resnet_layers=3, resnet_hidden=128, use_sn=False):
        super().__init__()
        self.C = C
        inputs = tf.keras.layers.Input(shape=(x_dim,))
        x = ResNetFeat(resnet_layers, resnet_hidden, use_sn=use_sn)(inputs)
        outputs = RandomFeatureGaussianProcess(C, gp_cov_momentum=gp_cov_momentum)(x)
        self.model = tf.keras.Model(inputs=inputs, outputs=outputs)
        self.model.build((None,x_dim))
        self.loss =  tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
        self.gp_mean_field_factor = np.pi / 8.

    def call(self, X, y, training):
        concat_feat = tf.concat([X[i] for i in range(len(X))], axis=1)
        logits = self.model(concat_feat, training=training)
