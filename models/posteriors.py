...
RegressionData = Tuple[TensorType, TensorType]
...
class Multimodal:
    ...
    def __init__(self, model) -> None:
        self.post_1 = IndependentPosteriorSingleOutput(model[0].kernel,
            model[0].inducing_variable,
            model[0].q_mu,
            model[0].q_sqrt,
            whiten=model[0].whiten,
            mean_function=None,
            precompute_cache=None,
            )
        self.post_2 = IndependentPosteriorSingleOutput(model[1].kernel,
            model[1].inducing_variable,
            model[1].q_mu,
            model[1].q_sqrt,
            whiten=model[1].whiten,
            mean_function=None,
            precompute_cache=None,
            )
    ...
    # could almost be the same as IndependentPosteriorMultiOutput ...
    def _conditional_fused(
        self, Xnew: TensorType, expert=0, full_cov: bool = False, full_output_cov: bool = False
    ) -> MeanAndVariance:
        if expert == 0:
            q_mu_1 = self.post_1.q_mu
            q_mu_2 = self.post_2.q_mu
            q_sqrt_2 = self.post_2.q_sqrt

            X_1 = self.post_1.X_data
            kernel_1 = self.post_1.kernel
            X_2 = self.post_2.X_data
            kernel_2 = self.post_2.kernel
        else:
            q_mu_1 = self.post_2.q_mu
            q_mu_2 = self.post_1.q_mu
            q_sqrt_2 = self.post_1.q_sqrt

            X_1 = self.post_2.X_data
            kernel_1 = self.post_2.kernel
            X_2 = self.post_1.X_data
            kernel_2 = self.post_1.kernel

        white=self.post_1.whiten,
        
        Kmm_1 = covariances.Kuu(X_1, kernel_1, jitter=default_jitter())  # [..., M, M]
        Kmn_1 = covariances.Kuf(X_1, kernel_1, Xnew) # [M, ..., N]
        Knn_1 = kernel_1(Xnew, full_cov=full_cov)  # [..., N] (full_cov = False) or [..., N, N] (True)

        Kmm_2 = covariances.Kuu(X_2, kernel_2, jitter=default_jitter()) # [..., M, M]
        Kmm_12 = covariances.Kuf(X_1, kernel_1, X_2.Z) # [M, ..., M]

        Lm_1 = tf.linalg.cholesky(Kmm_1)
        Lm_2 = tf.linalg.cholesky(Kmm_2)

        num_func = tf.shape(q_mu_1)[-1]  # R
        N = tf.shape(Kmn_1)[-1]
        M = tf.shape(q_mu_1)[-2]
        ...
        Kmn_1 = tf.transpose(Kmn_1, perm)  # [..., M, N]
        ...
        Kmm_12 = tf.transpose(Kmm_12, perm)  # [..., M, M]

        shape_constraints = [
            (Kmn_1, [..., "M", "N"]),
            (Lm_1, ["M", "M"]),
            (Knn_1, [..., "N", "N"] if full_cov else [..., "N"]),
            (q_mu_1, ["M", "R"]),
        ]
        if q_sqrt_2 is not None:
            shape_constraints.append(
                (q_sqrt_2, (["M", "R"] if q_sqrt_2.shape.ndims == 2 else ["R", "M", "M"]))
            )
        tf.debugging.assert_shapes(
            shape_constraints,
        ...
        leading_dims = tf.shape(Kmn_1)[:-2]

        # Compute the projection matrix B_T
        Lm_1 = tf.broadcast_to(Lm_1, tf.concat([leading_dims, tf.shape(Lm_1)], 0))  # [..., M, M]
        W_B = tf.linalg.triangular_solve(Lm_1, Kmn_1, lower=True)  # [..., M, N]

        if not white:
            B_T = tf.linalg.triangular_solve(tf.linalg.adjoint(Lm_1), W_B, lower=False)
        else:
            B_T = W_B
        
        # Compute the projection matrix W_C
        Lm_2 = tf.broadcast_to(Lm_2, tf.concat([leading_dims, tf.shape(Lm_2)], 0))  # [..., M, M]
        W_C = tf.linalg.triangular_solve(Lm_2, tf.linalg.matmul(Kmm_12, B_T, transpose_a=True), lower=True)  # [..., M, N]
        ...
        if full_cov:
            fvar = Knn_1 - tf.linalg.matmul(W_C, W_C, transpose_a=True)  # [..., N, N]
            cov_shape = tf.concat([leading_dims, [num_func, N, N]], 0)
            fvar = tf.broadcast_to(tf.expand_dims(fvar, -3), cov_shape)  # [..., R, N, N]
        else:
            fvar = Knn_1 - tf.reduce_sum(tf.square(W_C), -2)  # [..., N]
            cov_shape = tf.concat([leading_dims, [num_func, N]], 0)  # [..., R, N]
            fvar = tf.broadcast_to(tf.expand_dims(fvar, -2), cov_shape)  # [..., R, N]

        # another backsubstitution in the unwhitened case
        if not white:
            C_T = tf.linalg.triangular_solve(tf.linalg.adjoint(Lm_2), W_C, lower=False)
        else:
            C_T = W_C

        # construct the conditional mean
        f_shape = tf.concat([leading_dims, [M, num_func]], 0)  # [..., M, R]
        q_mu_2 = tf.broadcast_to(q_mu_2, f_shape)  # [..., M, R]
        fmean = tf.linalg.matmul(C_T, q_mu_2, transpose_a=True)  # [..., N, R]

        if q_sqrt_2 is not None:
            q_sqrt_dims = q_sqrt_2.shape.ndims
            if q_sqrt_dims == 2:
                LTC = C_T * tf.expand_dims(tf.transpose(q_sqrt_2), 2)  # [R, M, N]
            elif q_sqrt_dims == 3:
                L = tf.linalg.band_part(q_sqrt_2, -1, 0)  # force lower triangle # [R, M, M]
                L_shape = tf.shape(L)
                L = tf.broadcast_to(L, tf.concat([leading_dims, L_shape], 0))

                shape = tf.concat([leading_dims, [num_func, M, N]], axis=0)
                A_tiled = tf.broadcast_to(tf.expand_dims(C_T, -3), shape)
                LTC = tf.linalg.matmul(L, A_tiled, transpose_a=True)  # [R, M, N]
            else:  # pragma: no cover
                raise ValueError("Bad dimension for q_sqrt: %s" % str(q_sqrt_2.shape.ndims))

            if full_cov:
                fvar = fvar + tf.linalg.matmul(LTC, LTC, transpose_a=True)  # [R, N, N]
            else:
                fvar = fvar + tf.reduce_sum(tf.square(LTC), -2)  # [R, N]

        if not full_cov:
            fvar = tf.linalg.adjoint(fvar)  # [N, R]

        shape_constraints = [
            (Kmn_1, [..., "M", "N"]),  # tensor included again for N dimension
            (q_mu_2, [..., "M", "R"]),  # tensor included again for R dimension
            (fmean, [..., "N", "R"]),
            (fvar, [..., "R", "N", "N"] if full_cov else [..., "N", "R"]),
        ]
        tf.debugging.assert_shapes(shape_constraints, message="base_conditional() return values")
        if expert == 0:
            return self.post_1._post_process_mean_and_cov(fmean, fvar, full_cov, full_output_cov)
        else:
            return self.post_2._post_process_mean_and_cov(fmean, fvar, full_cov, full_output_cov)
